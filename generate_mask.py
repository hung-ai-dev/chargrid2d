...
class MaskGenerator():
    ...
    def convert_lbl(self, lbl_fol, out_fol):
        ...
        for idx, file_path in enumerate(all_files):
            ...
            write_json(osp.join(out_fol, name), lbl_data)
            self.path_std_lbls.append(osp.join(out_fol, name))
    ...
    def generate_mask(self, out_fol):
        ...
        for lbl_path, img_path in zip(self.path_std_lbls, self.path_imgs):
            ...
            input_mask = np.zeros((doc_h, doc_w), dtype='int16')
            ...
            for item in lbl_data:
                ...
                for char in item['value']:
                    ...
                    # print(mask[cur_y: cur_y + char_h, cur_x: cur_x + char_w])
                    cur_x += char_w
            ...
            cv2.imwrite(osp.join('./data/semantic_gt', name + '.png'), gt)
            write_json(osp.join('./data/obj_gt', name + '.json'), obj)
            torch.save(tensor, osp.join('./data/tensor_input', name + '.pt'))
